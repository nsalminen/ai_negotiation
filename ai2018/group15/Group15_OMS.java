...
public class Group15_OMS extends OMStrategy {
	...
	/**
	 * When to stop updating the opponent model. Note that this value is not exactly
	...
	/**
	 * Initializes the opponent model strategy. If values for the parameter t and n
	 * are given, then it is set to this value. Otherwise, the default values are
	 * used.
	 * 
	 * @param negotiationSession state of the negotiation.
	 * @param model              opponent model used in conjunction with this
	 *                           opponent modelling strategy.
	...
	public BidDetails getBid(List<BidDetails> allBids) {
		...
		/**
		 * We need to have an array with the opponents bids and their utility for the
		 * opponent according to our opponent model, so we can sort them later.
		 */
		ArrayList<BidDetails> opponentBids = new ArrayList<BidDetails>(allBids.size());

		/**
		 * For each opponent bid, evaluate them according to the opponent model and put
		 * them into the array.
		 */
		...
		/**
		 * Use the BidDetailsSorter Utility to then sort these bids The bid with the
		 * highest utility is at the front of the list.
		 */
		Collections.sort(opponentBids, comparer);

		/**
		 * Now randomly select a bid from the top N. (Indexing starts at 0 so we want to
		 * generate random numbers from 0 to N-1 for the top N)
		 */
		...
		BidDetails bestBid = new BidDetails(bestOpponentBid, utilitySpace.getUtility(bestOpponentBid),
